...
========================================================
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      {/* HEADER SECTION */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-farmer-border/80 pb-5">
        <div>
          <p className="text-[11px] font-extrabold text-farmer-primary uppercase tracking-widest">{t('centre.portalTitle', 'Procurement Centre')}</p>
          <h1 className="text-2xl md:text-3xl font-black text-farmer-text tracking-tight mt-0.5">{t('reports.title', 'Operational Reports & Analytics')}</h1>
          <p className="text-sm font-medium text-farmer-secondary mt-1">{t('reports.subtitle', 'Real-time procurement statistics, farmer throughput and payment ledgers')}</p>
        </div>

        <button
          onClick={handleExportPDF}
          className="bg-farmer-primary hover:bg-[#03522c] text-white font-extrabold text-xs px-5 py-3 rounded-2xl shadow-xs transition-colors flex items-center gap-2 cursor-pointer"
        >
      ...
      <div className="flex gap-2 border-b border-farmer-border/90 overflow-x-auto custom-scrollbar pb-1">
        {[
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2.5 px-4 py-3 text-xs font-extrabold border-b-2 transition-all cursor-pointer whitespace-nowrap ${
                isActive
                  ? 'border-farmer-primary text-farmer-primary bg-farmer-primary-light/40 rounded-t-xl'
                  : 'border-transparent text-farmer-secondary hover:text-farmer-text hover:bg-slate-50 rounded-t-xl'
              }`}
            >
              <Icon className={`w-4 h-4 ${isActive ? 'text-farmer-primary' : 'text-farmer-secondary'}`} strokeWidth={2.5} />
              <span>{tab.label}</span>
            </button>
          );
        })}
      ...
      <Card className="border border-farmer-border/80 shadow-xs bg-white rounded-2xl overflow-hidden">
        <CardContent className="p-4 md:p-5 space-y-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-farmer-primary-light text-farmer-primary flex items-center justify-center">
                <Filter className="w-4 h-4" strokeWidth={2.5} />
              </div>
              <div>
                <h3 className="font-extrabold text-farmer-text text-sm">{t('reports.filterTitle', 'Report Filters')}</h3>
                <p className="text-[11px] font-medium text-farmer-secondary">{t('reports.filterSubtitle', 'Filter real procurement records by Date, Crop, Status and Farmer Search')}</p>
              </div>
            </div>

            <button
              onClick={handleClearFilters}
              className="flex items-center gap-1.5 text-xs font-bold text-farmer-secondary hover:text-farmer-text transition-colors cursor-pointer"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 pt-1">
            {/* Date Range */}
            <div>
              <label className="text-[11px] font-extrabold text-farmer-secondary uppercase tracking-wider block mb-1">{t('reports.filterDateRange', 'Date Range')}</label>
              <select
            ...
            {/* Commodity */}
            <div>
              <label className="text-[11px] font-extrabold text-farmer-secondary uppercase tracking-wider block mb-1">{t('reports.filterCommodity', 'Commodity')}</label>
              <select
            ...
            {/* Status */}
            <div>
              <label className="text-[11px] font-extrabold text-farmer-secondary uppercase tracking-wider block mb-1">{t('reports.filterStatus', 'Status')}</label>
              <select
            ...
            {/* Farmer Search */}
            <div>
              <label className="text-[11px] font-extrabold text-farmer-secondary uppercase tracking-wider block mb-1">{t('reports.filterSearch', 'Farmer Search')}</label>
              <div className="relative">
                <Search className="w-3.5 h-3.5 absolute left-3 top-1/2 -translate-y-1/2 text-farmer-secondary" />
                <input
                  type="text"
                  placeholder="Search farmer name / ID / token"
                  value={filterForm.search}
                  onChange={(e) => setFilterForm(prev => ({ ...prev, search: e.target.value }))}
                  className="w-full h-10 border border-farmer-border rounded-xl pl-9 pr-3 text-xs font-bold text-farmer-text bg-white focus:outline-none focus:border-farmer-primary"
                />
              </div>
            </div>
            ...
            <div className="flex items-end">
              <button
                ...
                className="w-full h-10 bg-farmer-primary hover:bg-[#03522c] text-white font-extrabold text-xs rounded-xl shadow-xs transition-colors cursor-pointer"
              >
          ...
          {filterForm.dateRange === 'custom' && (
            <div className="flex flex-wrap items-center gap-4 pt-2 border-t border-farmer-border text-xs font-bold">
              <div>
                <label className="text-farmer-secondary mr-2">Start Date:</label>
                <input 
                  type="date" 
                  value={filterForm.startDate} 
                  onChange={(e) => setFilterForm(prev => ({ ...prev, startDate: e.target.value }))}
                  className="border border-farmer-border rounded-lg px-2 py-1 text-farmer-text focus:outline-none focus:border-farmer-primary" 
                />
              </div>
              <div>
                <label className="text-farmer-secondary mr-2">End Date:</label>
                <input 
                  type="date" 
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <Card 
              ...
              className="border border-farmer-border/80 shadow-xs bg-white rounded-2xl hover:border-farmer-primary transition-all cursor-pointer group overflow-hidden"
            >
              <CardContent className="p-5 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xs font-bold text-farmer-secondary">{t('reports.farmersServed', 'Farmers Served')}</p>
                    <h3 className="text-2xl font-black text-farmer-text mt-0.5">{kpiData.farmersServed}</h3>
                    <p className="text-[11px] font-bold text-emerald-700 mt-1 flex items-center gap-1">
                      <CheckCircle2 className="w-3 h-3 text-farmer-primary" /> Calculated from filtered records
                    </p>
                  </div>
                </div>
                <ChevronRight className="w-5 h-5 text-farmer-secondary group-hover:text-farmer-primary group-hover:translate-x-1 transition-all" />
              </CardContent>
            </Card>
            ...
            <Card className="border border-farmer-border/80 shadow-xs bg-white rounded-2xl overflow-hidden">
              <CardContent className="p-5 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xs font-bold text-farmer-secondary">{t('reports.totalQuantity', 'Total Quantity Procured')}</p>
                    <h3 className="text-2xl font-black text-farmer-text mt-0.5">{kpiData.totalQuantityQ} Q</h3>
                    <p className="text-[11px] font-medium text-farmer-secondary mt-1">{kpiData.totalQuantityKg.toLocaleString('en-IN')} kg total net weight</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border border-farmer-border/80 shadow-xs bg-white rounded-2xl overflow-hidden">
              <CardContent className="p-5 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xs font-bold text-farmer-secondary">{t('reports.procurementValue', 'Procurement Value')}</p>
                    <h3 className="text-2xl font-black text-farmer-text mt-0.5">₹{kpiData.totalProcurementValue.toLocaleString('en-IN')}</h3>
                    <p className="text-[11px] font-medium text-farmer-secondary mt-1">Total MSP value disbursed</p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <Card className="lg:col-span-7 border border-farmer-border/80 shadow-xs bg-white rounded-2xl overflow-hidden">
              <CardHeader className="bg-white border-b border-farmer-border py-4 px-5 flex justify-between items-center">
                <CardTitle className="text-sm font-extrabold text-farmer-text flex items-center gap-2">
                  <BarChart3 className="w-4 h-4 text-farmer-primary" strokeWidth={2.5} />
                  {t('reports.procurementTrend', 'Procurement Trend')}
                </CardTitle>
                <div className="flex items-center gap-4 text-xs font-bold">
                  <div className="flex items-center gap-1.5">
                    <span className="w-3 h-3 rounded-sm bg-farmer-primary"></span>
                    <span className="text-farmer-secondary">Quantity (Q)</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="w-3 h-3 rounded-sm bg-[#fbbf24]"></span>
                    <span className="text-farmer-secondary">Farmers</span>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="p-6">
                {trendData.length > 0 ? (
                    ...
                    return (
                      <div className="h-60 flex flex-col justify-between relative pt-6">
                        ...
                        <div className="absolute inset-x-0 top-6 bottom-8 flex flex-col justify-between pointer-events-none">
                          <div className="border-b border-farmer-border border-dashed w-full flex justify-between text-[10px] text-farmer-secondary font-bold -mt-2.5">
                            <span>{maxQty} Q</span>
                            <span>{maxFarmers} Farmers</span>
                          </div>
                          <div className="border-b border-farmer-border border-dashed w-full flex justify-between text-[10px] text-slate-300 font-bold -mt-2.5">
                            <span>{Math.round(maxQty / 2)} Q</span>
                            <span>{Math.ceil(maxFarmers / 2)} Farmers</span>
                          </div>
                          <div className="border-b border-farmer-border w-full flex justify-between text-[10px] text-farmer-secondary font-bold -mt-2.5">
                            <span>0 Q</span>
                            <span>0</span>
                          </div>
                        </div>
                              ...
                              <div key={idx} className="flex-1 flex flex-col items-center h-full justify-end group relative max-w-[64px]">
                                ...
                                <div className="flex items-end gap-1.5 w-full justify-center h-full">
                                  ...
                                  <div 
                                    ...
                                    className="w-3.5 bg-farmer-primary rounded-t-sm transition-all group-hover:bg-[#03522c] shadow-xs"
                                  ></div>
                                  {/* FARMERS BAR */}
                                  <div 
                                ...
                                {/* X-AXIS LABEL */}
                                <span className="absolute -bottom-6 text-[10px] font-bold text-farmer-secondary tracking-tight whitespace-nowrap">
                                  {item.dateLabel}
                                </span>
                              </div>
                            );
                  ...
                  <div className="h-56 flex flex-col items-center justify-center text-center p-6 text-farmer-secondary font-bold text-xs">
            ...
            <Card className="lg:col-span-5 border border-farmer-border/80 shadow-xs bg-white rounded-2xl overflow-hidden">
              <CardHeader className="bg-white border-b border-farmer-border py-4 px-5">
                <CardTitle className="text-sm font-extrabold text-farmer-text flex items-center gap-2">
                  <PieChart className="w-4 h-4 text-farmer-primary" strokeWidth={2.5} />
              ...
              <CardContent className="p-6 flex flex-col sm:flex-row items-center justify-between gap-6">
                ...
                {cropDistribution.length > 0 ? (
                  <>
                    ...
                    <div className="relative w-44 h-44 flex items-center justify-center shrink-0">
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                        <span className="text-xl font-black text-farmer-text leading-tight">{kpiData.totalQuantityQ} Q</span>
                        <span className="text-[10px] font-extrabold text-farmer-primary uppercase tracking-wider">Total</span>
                      </div>
                    </div>
                      ...
                      {cropDistribution.map((crop, idx) => (
                        <div key={idx} className="flex items-center justify-between text-xs">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-bold text-farmer-text">{crop.name}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-[11px] font-semibold text-farmer-secondary">{(crop.quantityKg / 100).toFixed(1)} Q</span>
                            <span className="font-black text-farmer-text w-9 text-right">{crop.percentage}%</span>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="w-full py-8 text-center text-xs font-bold text-farmer-secondary">
          ...
          <Card className="border border-farmer-border/80 shadow-xs bg-white rounded-2xl overflow-hidden">
            <CardHeader className="bg-white border-b border-farmer-border py-4 px-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <div>
                <CardTitle className="text-sm font-extrabold text-farmer-text flex items-center gap-2">
                  <Users className="w-4 h-4 text-farmer-primary" strokeWidth={2.5} />
                  {t('reports.recentFarmersServed', 'Recent Farmers Served')}
                </CardTitle>
                <p className="text-[11px] font-medium text-farmer-secondary mt-0.5">Showing verified procurement records derived from applied filters</p>
              </div>

              <button
                onClick={() => handleTabChange('farmers-served')}
                className="text-xs font-extrabold text-farmer-primary hover:underline flex items-center gap-1 cursor-pointer"
              >
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50/80 border-b border-farmer-border/70 text-farmer-secondary uppercase font-black text-[10px] tracking-wider">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-100 bg-white text-xs font-semibold">
                  {filteredRecords.length > 0 ? (
                    filteredRecords.slice(0, 5).map((rec, idx) => (
                      ...
                      >
                        <td className="p-3.5 pl-5 text-farmer-secondary font-bold">{idx + 1}</td>
                        <td className="p-3.5 font-black text-farmer-primary">{rec.token}</td>
                        <td className="p-3.5 font-bold text-farmer-text">{rec.farmerName}</td>
                        <td className="p-3.5 text-farmer-secondary font-mono">{rec.farmerId}</td>
                        <td className="p-3.5 font-bold text-farmer-text">{rec.commodity}</td>
                        <td className="p-3.5 font-bold text-farmer-text">{rec.quantity} kg</td>
                        <td className="p-3.5 font-black text-farmer-text">₹{rec.amount.toLocaleString('en-IN')}</td>
                        <td className="p-3.5 text-farmer-secondary">{rec.dateTime}</td>
                        <td className="p-3.5 pr-5">
                          <span className="bg-farmer-primary-light text-farmer-primary px-2.5 py-0.5 rounded-full text-[11px] font-extrabold border border-emerald-200">
                            {rec.status}
                          </span>
                        </td>
                      </tr>
                    ...
                    <tr>
                      <td colSpan="9" className="p-8 text-center text-farmer-secondary font-bold">
            ...
            <div className="p-4 border-t border-farmer-border flex justify-between items-center text-xs font-bold text-farmer-secondary">
              <span>Showing {Math.min(5, filteredRecords.length)} of {filteredRecords.length} records</span>
              <button 
                onClick={() => handleTabChange('farmers-served')}
                className="text-farmer-primary hover:underline cursor-pointer"
              >
                View full dataset →
              </button>
            </div>
                ...
                {filteredRecords.length > 0 ? (
                  filteredRecords.map((rec, idx) => (
                    <tr key={rec.id} className="hover:bg-slate-50 transition-colors">
                      <td className="p-3.5 pl-5 text-farmer-secondary">{idx + 1}</td>
                      <td className="p-3.5 font-black text-farmer-primary">{rec.token}</td>
                      <td className="p-3.5 font-bold text-farmer-text">{rec.farmerName}</td>
                      <td className="p-3.5 text-farmer-secondary font-mono">{rec.farmerId}</td>
                      <td className="p-3.5 text-farmer-secondary">{rec.phone}</td>
                      <td className="p-3.5 font-bold text-farmer-text">{rec.commodity}</td>
                      <td className="p-3.5 font-bold text-farmer-text">{rec.quantity} kg</td>
                      <td className="p-3.5 font-black text-farmer-primary">₹{rec.amount.toLocaleString('en-IN')}</td>
                      <td className="p-3.5 text-farmer-secondary">{rec.dateTime}</td>
                      <td className="p-3.5 pr-5">
                        <button
                          onClick={() => setSelectedFarmerRecord(rec)}
                          className="p-1.5 rounded-lg bg-farmer-primary-light text-farmer-primary hover:bg-farmer-primary hover:text-white transition-colors cursor-pointer"
                  ...
                  <tr>
                    <td colSpan="10" className="p-8 text-center text-farmer-secondary font-bold">
                      {t('reports.noRecordsFound', 'No procurement records match the applied filters.')}
                    </td>
                  </tr>
                )}
              ...
              <tbody className="divide-y divide-slate-100 bg-white text-xs font-semibold">
                {filteredRecords.length > 0 ? (
                  filteredRecords.map((rec) => (
                    <tr key={rec.id} className="hover:bg-slate-50 transition-colors">
                      <td className="p-3.5 pl-5 font-bold text-farmer-primary font-mono">{rec.id}</td>
                      <td className="p-3.5 font-black text-farmer-text">{rec.token}</td>
                      <td className="p-3.5 font-bold text-farmer-text">{rec.farmerName}</td>
                      <td className="p-3.5 font-bold text-farmer-text">{rec.commodity} ({rec.variety})</td>
                      <td className="p-3.5 font-black text-farmer-text">{rec.quantity} kg</td>
                      <td className="p-3.5"><span className="bg-slate-100 text-farmer-text font-bold px-2 py-0.5 rounded">{rec.grade}</span></td>
                      <td className="p-3.5 font-bold text-farmer-text">₹{rec.rate}</td>
                      <td className="p-3.5 font-black text-farmer-text">₹{rec.amount.toLocaleString('en-IN')}</td>
                      <td className="p-3.5 pr-5">
                        <button
                          onClick={() => setSelectedFarmerRecord(rec)}
                          className="px-3 py-1 bg-farmer-primary text-white rounded-lg text-[11px] font-bold hover:bg-[#03522c] transition-colors cursor-pointer"
                        >
                          View Receipt
                        </button>
                      </td>
          ...
          {cropDistribution.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {cropDistribution.map((c, i) => (
                <Card key={i} className="border border-farmer-border/80 shadow-xs bg-white rounded-2xl overflow-hidden p-5">
                  <div className="flex justify-between items-center mb-3">
                    <span className="font-extrabold text-farmer-text text-base">{c.name}</span>
                    <span className="w-3.5 h-3.5 rounded-full" style={{ backgroundColor: c.color }}></span>
                  </div>
                  <div className="space-y-1">
                    <p className="text-xs font-medium text-farmer-secondary">Share of Total Procurement</p>
                    <h3 className="text-2xl font-black text-farmer-text">{c.percentage}%</h3>
                    <p className="text-xs font-bold text-farmer-secondary mt-1">{c.quantityKg.toLocaleString('en-IN')} kg total</p>
                  </div>
                </Card>
              ))}
            </div>
          ) : (
            <Card className="p-8 text-center text-farmer-secondary font-bold">
              {t('reports.noRecordsFound', 'No procurement records match the applied filters.')}
            </Card>
          )}
        </div>
        ...
        <div className="space-y-6">
          <Card className="border border-farmer-border/80 shadow-xs bg-white rounded-2xl overflow-hidden">
            <CardHeader className="bg-white border-b border-farmer-border py-4 px-5 flex justify-between items-center">
              <div>
                <CardTitle className="text-sm font-extrabold text-farmer-text flex items-center gap-2">
                  <CreditCard className="w-4 h-4 text-farmer-primary" strokeWidth={2.5} />
                  {t('reports.disbursementLedger', 'Farmer Disbursements & Payments Ledger')}
                </CardTitle>
                <p className="text-[11px] font-medium text-farmer-secondary mt-0.5">{t('reports.disbursementSub', 'Manage and track payment disbursements to verified farmers')}</p>
              </div>
            </CardHeader>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 bg-white text-xs font-semibold">
                  {filteredRecords.length > 0 ? (
                    filteredRecords.map((rec) => (
                      <tr key={rec.id} className="hover:bg-slate-50 transition-colors">
                        <td className="p-3.5 pl-5 font-black text-farmer-primary">{rec.token}</td>
                        <td className="p-3.5 font-bold text-farmer-text">
                          {rec.farmerName}
                          <span className="text-[10px] text-farmer-secondary block font-mono">{rec.farmerId}</span>
                        </td>
                        <td className="p-3.5">
                          <span className="font-bold text-farmer-text block">{rec.bankName}</span>
                          <span className="text-[10px] font-mono text-farmer-secondary block">{rec.accountNumber} ({rec.ifsc})</span>
                        </td>
                        <td className="p-3.5 font-bold text-farmer-text">{rec.commodity} ({rec.quantity} kg)</td>
                        <td className="p-3.5 font-black text-farmer-text">₹{rec.amount.toLocaleString('en-IN')}</td>
                        <td className="p-3.5">
                          <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-extrabold border ${
                            rec.paymentStatus === 'Paid'
                              ? 'bg-emerald-50 text-farmer-primary border-emerald-200'
                              : 'bg-amber-50 text-amber-800 border-amber-200'
                          }`}>
                            {rec.paymentStatus}
                          </span>
                        </td>
                        <td className="p-3.5 font-mono text-farmer-secondary">{rec.transactionRef || '—'}</td>
                        <td className="p-3.5 pr-5">
                          <button
                            onClick={() => {
                            ...
                            className="px-3 py-1 bg-farmer-primary text-white font-bold rounded-lg text-xs hover:bg-[#03522c] transition-colors cursor-pointer"
                          >
                    ...
                    <tr>
                      <td colSpan="8" className="p-8 text-center text-farmer-secondary font-bold">
                        {t('reports.noRecordsFound', 'No procurement records match the applied filters.')}
                      </td>
                    </tr>
                  )}
      ...
      {selectedFarmerRecord && (
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-xl w-full shadow-2xl overflow-hidden animate-in zoom-in-95 border border-farmer-border">
            {/* Modal Header */}
            ...
            {/* Modal Body */}
            <div className="p-6 space-y-5 text-xs font-semibold text-farmer-text max-h-[80vh] overflow-y-auto custom-scrollbar">
              <div className="bg-farmer-primary-light p-4 rounded-xl border border-emerald-200 flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-extrabold text-farmer-primary uppercase tracking-wider">Total Procurement Amount</p>
                  <p className="text-2xl font-black text-farmer-text">₹{selectedFarmerRecord.amount.toLocaleString('en-IN')}</p>
                </div>
                <div className="text-right">
                  <span className="bg-farmer-primary text-white px-3 py-1 rounded-full text-xs font-extrabold">
              ...
              <div className="space-y-2 bg-slate-50 p-4 rounded-xl border border-farmer-border/80">
                <p className="text-[10px] font-extrabold text-farmer-secondary uppercase tracking-widest mb-1">Farmer & Location</p>
                <div className="grid grid-cols-2 gap-3">
                  <div><span className="text-farmer-secondary font-normal">Phone:</span> <span className="font-bold text-farmer-text block">{selectedFarmerRecord.phone}</span></div>
                  <div><span className="text-farmer-secondary font-normal">Location:</span> <span className="font-bold text-farmer-text block">{selectedFarmerRecord.location}</span></div>
                  <div><span className="text-farmer-secondary font-normal">Date & Time:</span> <span className="font-bold text-farmer-text block">{selectedFarmerRecord.dateTime}</span></div>
                  <div><span className="text-farmer-secondary font-normal">Procurement ID:</span> <span className="font-bold text-farmer-text block font-mono">{selectedFarmerRecord.id}</span></div>
                </div>
              </div>

              <div className="space-y-2 bg-slate-50 p-4 rounded-xl border border-farmer-border/80">
                <p className="text-[10px] font-extrabold text-farmer-secondary uppercase tracking-widest mb-1">Crop & Quality Parameters</p>
                <div className="grid grid-cols-3 gap-3">
                  <div><span className="text-farmer-secondary font-normal">Commodity:</span> <span className="font-bold text-farmer-text block">{selectedFarmerRecord.commodity}</span></div>
                  <div><span className="text-farmer-secondary font-normal">Variety:</span> <span className="font-bold text-farmer-text block">{selectedFarmerRecord.variety}</span></div>
                  <div><span className="text-farmer-secondary font-normal">Net Quantity:</span> <span className="font-bold text-farmer-text block">{selectedFarmerRecord.quantity} kg</span></div>
                  <div><span className="text-farmer-secondary font-normal">Rate (₹/kg):</span> <span className="font-bold text-farmer-text block">₹{selectedFarmerRecord.rate}</span></div>
                  <div><span className="text-farmer-secondary font-normal">Moisture:</span> <span className="font-bold text-farmer-text block">{selectedFarmerRecord.moisture}</span></div>
                  <div><span className="text-farmer-secondary font-normal">Grain Grade:</span> <span className="font-bold text-farmer-text block">{selectedFarmerRecord.grade}</span></div>
                </div>
              </div>

              <div className="space-y-2 bg-slate-50 p-4 rounded-xl border border-farmer-border/80">
                <p className="text-[10px] font-extrabold text-farmer-secondary uppercase tracking-widest mb-1">Bank Disbursement Details</p>
                <div className="grid grid-cols-2 gap-3">
                  <div><span className="text-farmer-secondary font-normal">Bank Name:</span> <span className="font-bold text-farmer-text block">{selectedFarmerRecord.bankName}</span></div>
                  <div><span className="text-farmer-secondary font-normal">Account Number:</span> <span className="font-bold text-farmer-text block">{selectedFarmerRecord.accountNumber}</span></div>
                  <div><span className="text-farmer-secondary font-normal">IFSC Code:</span> <span className="font-bold text-farmer-text block">{selectedFarmerRecord.ifsc}</span></div>
                  <div><span className="text-farmer-secondary font-normal">UPI ID:</span> <span className="font-bold text-farmer-text block">{selectedFarmerRecord.upi}</span></div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl overflow-hidden animate-in zoom-in-95 border border-farmer-border">
            ...
            <div className="p-6 space-y-5 text-xs font-semibold text-farmer-text">
              <div className="bg-farmer-primary-light p-4 rounded-xl border border-emerald-200">
                <p className="text-[10px] font-extrabold text-farmer-primary uppercase tracking-wider">Farmer</p>
                <p className="font-black text-farmer-text text-base">{selectedPaymentRecord.farmerName}</p>
                <p className="text-farmer-secondary font-medium text-xs">Amount: ₹{selectedPaymentRecord.amount.toLocaleString('en-IN')}</p>
              </div>

              <div>
                <label className="text-xs font-bold text-farmer-text block mb-1.5">{t('reports.transRef', 'Transaction Reference ID (UTR)')}</label>
                <input
                  type="text"
                  placeholder="e.g. UTR-908234105"
                  value={paymentTransactionRef}
                  onChange={(e) => setPaymentTransactionRef(e.target.value)}
                  className="w-full h-11 border border-farmer-border rounded-xl px-3 text-xs font-bold font-mono text-farmer-text uppercase focus:outline-none focus:border-farmer-primary"
                />
              </div>
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-3 bg-farmer-primary hover:bg-[#03522c] text-white font-extrabold rounded-xl transition-colors cursor-pointer shadow-xs"
                >
                  Mark as Paid
                </button>
                <button
