    ...
    <div className="space-y-6 font-sans">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight mt-0.5">Complaints & Feedback</h1>
          <p className="text-sm font-medium text-slate-500 mt-1">Manage issues raised by farmers at this centre.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
         <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-xs">
            <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-1">Under Review</p>
            <h3 className="text-2xl md:text-3xl font-black text-amber-600">{(state.feedback || []).filter(c=>c.status==='Under Review').length}</h3>
         </div>
         <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-xs">
            <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-1">Resolved Today</p>
            <h3 className="text-2xl md:text-3xl font-black text-[#046a38]">{(state.feedback || []).filter(c=>c.status==='Resolved').length}</h3>
         </div>
      </div>
              ...
              {filteredComplaints.length > 0 ? filteredComplaints.map((c) => {
                ...
                return (
                  <tr key={c.id} className="hover:bg-slate-50/70 transition-colors">
                    <td className="p-4">
                       <p className="font-bold text-[#046a38] text-sm">{c.id}</p>
                       <p className="text-[10px] font-bold text-slate-400 mt-0.5 uppercase tracking-wider">{new Date(c.date).toLocaleDateString()}</p>
                    </td>
                    <td className="p-4 font-bold text-slate-900">{c.category}</td>
                    <td className="p-4">
                       <p className="font-bold text-slate-900">{farmer?.name}</p>
                       <p className="text-xs font-medium text-slate-500">{farmer?.mobile}</p>
                    </td>
                    <td className="p-4 text-sm font-medium text-slate-600 max-w-xs truncate">{c.description}</td>
                    <td className="p-4">
                       <span className={`px-2.5 py-1 rounded-full uppercase font-extrabold tracking-wider text-[10px] border ${
                         c.status === 'Resolved' ? 'bg-[#e6f4ea] text-[#046a38] border-emerald-200' :
                    ...
                    <td className="p-4 text-right">
                       <Button size="sm" variant="outline" className="h-8 font-bold text-xs shadow-xs bg-white border-slate-200 text-slate-700 rounded-xl hover:bg-slate-50" onClick={() => setSelectedComplaint(c)}>
                         Review
                       </Button>
                    </td>
                  </tr>
                ...
                <tr>
                  <td colSpan="6" className="p-8 text-center">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-slate-500 font-bold text-sm">No complaints found.</p>
                    </div>
                  </td>
                </tr>
              )}
      ...
      {selectedComplaint && (
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4">
           <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[90vh]">
              <div className="bg-[#046a38] p-5 flex justify-between items-center text-white shrink-0">
              ...
              <div className="p-6 space-y-6 overflow-y-auto">
                 ...
                 <div className="flex justify-between items-start">
                    <div>
                      <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-1">Complaint ID</p>
                      <h3 className="text-2xl font-black text-slate-900">{selectedComplaint.id}</h3>
                    </div>
                    <span className={`px-3 py-1 rounded-full uppercase font-extrabold tracking-wider text-[10px] border ${
                         selectedComplaint.status === 'Resolved' ? 'bg-[#e6f4ea] text-[#046a38] border-emerald-200' :
                 ...
                 <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-2">
                    ...
                    <p className="text-sm font-medium text-slate-800">{selectedComplaint.description}</p>
                 </div>

                 <div>
                    <label className="text-xs font-bold text-slate-700 uppercase tracking-wider block mb-2">Resolution Note</label>
                    <textarea 
                      className="w-full h-24 border border-slate-200 rounded-xl p-3 text-sm font-medium focus:outline-none focus:border-[#046a38] resize-none"
                 ...
                 <div className="flex gap-3 pt-2">
                    ...
                    <Button className="flex-1 font-bold bg-[#046a38] hover:bg-[#03522c] text-white rounded-xl shadow-xs cursor-pointer" onClick={() => handleUpdateStatus('Resolved')}>
