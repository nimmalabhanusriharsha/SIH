  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-farmer-text tracking-tight">Staff Directory</h2>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button className="bg-farmer-primary hover:bg-farmer-primary text-white border-0 shrink-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
         {filteredStaff.map(member => (
           <Card key={member.id} className="border-farmer-border shadow-sm bg-white hover:shadow-md transition-shadow">
              <CardContent className="p-5">
                 <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                       <div className="w-12 h-12 rounded-full bg-earth-100 flex items-center justify-center text-earth-700 font-bold border border-farmer-border">
                         {member.name.charAt(0)}
                       </div>
                       <div>
                         <h3 className="font-bold text-farmer-text">{member.name}</h3>
                         <p className="text-xs text-farmer-secondary font-mono">{member.id}</p>
                       </div>
                    </div>
                 ...
                 <div className="space-y-2 mb-4">
                    <p className="text-sm text-earth-700 flex items-center gap-2 font-medium bg-farmer-bg px-2 py-1.5 rounded-lg border border-farmer-border">
                      <ShieldCheck className="w-4 h-4 text-forest-500" /> {member.role}
                    </p>
                    <p className="text-sm text-earth-600 flex items-center gap-2">
                      <MapPin className="w-4 h-4 text-farmer-secondary" /> Assigned to <strong>{member.centreId}</strong>
                    </p>
                    <p className="text-sm text-earth-600 flex items-center gap-2">
                      <Phone className="w-4 h-4 text-farmer-secondary" /> {member.phone}
                    </p>
                    <p className="text-sm text-earth-600 flex items-center gap-2">
                      <Mail className="w-4 h-4 text-farmer-secondary" /> {member.email}
                    </p>
                 </div>
                 
                 <div className="grid grid-cols-2 gap-2">
                    <Button 
                      variant="outline" size="sm" className="bg-white border-farmer-border"
                      onClick={() => setSelectedStaff(member)}
                    >
                      Edit Details
                    </Button>
                    <Button 
                      variant="outline" size="sm" className="bg-white border-farmer-border text-red-600 hover:bg-red-50 hover:text-red-700"
          ...
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col">
            <div className="px-6 py-4 border-b border-farmer-border flex items-center justify-between bg-farmer-bg">
              <h3 className="font-bold text-lg text-farmer-text flex items-center gap-2">
                <ShieldCheck className="w-5 h-5 text-farmer-primary" /> Edit Staff Details
              </h3>
              <button onClick={() => setSelectedStaff(null)} className="p-1 hover:bg-earth-200 rounded-md text-farmer-secondary">✕</button>
            </div>
            ...
            <form onSubmit={handleSaveEdit}>
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <select 
                    className="w-full border border-farmer-border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-forest-500"
              ...
              <div className="p-4 border-t border-farmer-border bg-farmer-bg flex justify-end gap-3">
                <Button type="button" variant="ghost" onClick={() => setSelectedStaff(null)}>Cancel</Button>
                <Button type="submit" className="bg-farmer-primary hover:bg-farmer-primary text-white">Save Changes</Button>
              </div>
            </form>
          </div>
        </div>
